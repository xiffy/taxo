...
(function(cbe, $) {
	...
	cbe.Utils = {
		...
		if (doc && doc.hasAttributes()) {
			...
			if (typeof attribute == 'object'
				&& attribute.value == val
				&& attribute.name.lastIndexOf("xmlns:", 0) == 0) {
	...
	cbe.InlineDoc = function(inlineDoc) {
		...
		for ( var k in _namespaces.inline) {
		    var prefix = self.nsLookupByValue(_namespaces.inline[k],
			    true);
		    if (prefix.length) {
		...
		var inlineNameSpaceElements = Object.keys(self
		...
		_inlineDoc
			...
			.each(
				function(index, element) {
				    ...
				    if (nodeName.substring(0, this.nodeName
					    .indexOf(':')) == self.inlinePrefix) {

					if (nodeName == self.inlinePrefix + ':'
						+ 'header') {

					    _cache.header = node;

					} else if (nodeName == self.inlinePrefix
						+ ':' + 'hidden') {
						...
						.parents(self.inlinePrefix
							+ '\\:header, header').length == 0) {

					    // numeric and text elements are
					    // kept together because there is a
					    // requirement to follow the order
					    // in the inline document
					    if ((nodeName == self.inlinePrefix
						    + ':nonfraction')
						    || nodeName == self.inlinePrefix
							    + ':nonnumeric') {
		...
		Object
			...
			.forEach(
				function(currentElement) {
				    var regexReturnsFalseIfRelativeUrl = new RegExp(
					    '^(?:[a-z]+:)?//', 'i');

				    if (!regexReturnsFalseIfRelativeUrl
					    .test(queryAry[currentElement])) {

					permittedUrl = true;

				    } else {
					if (queryAry[currentElement]
						.startsWith('https://sec.gov/')						
						|| queryAry[currentElement]
							.startsWith('https://www.sec.gov/')) {
					    permittedUrl = true;
					} else {
					    queryThatIsNotPermitted = queryAry[currentElement];
					    permittedUrl = queryAry[currentElement]
		...
		if (!permittedUrl) {
		    ...
		    App
			    .showMessage("The Filing you are looking for is not there, please search EDGAR.");
		    App.hideLoadingDialog();
		    throw new CustomCORSError(null, window.location.origin,
			    queryThatIsNotPermitted);
		    return;
		}
		if (queryAry['metalinks']) {
		    metalinksUrl = queryAry['metalinks'].toString();
		    metalinksUrl.replace('interpretedFormat=true',
		...
		if (docUrl.indexOf('?') > 0) {
			...
			metalinksUrl = (subquery['filename'] = 'MetaLinks.json')
				.toString();
		    }
		} else {
	  ...
	  var _initCacheNewMore = function(parent, cnamegetting, callback) {
		...
		$
			.ajax({
			    ...
			    cache : true,
			    error : function(requestObject, error, errorThrown) {
				// App.showMessage("Could not locate valid <a
				// href='"+metamore+"'>metamore</a> document.");
				// App.hideLoadingDialog();
				...
				if (requestObject.version == "2.0") {
				    ...
				    App
					    .showMessage("Object found was not a Metamore version 2.0 file");
				}
			    }
		...
		if (types instanceof Array) {
			...
			// internal HTML representation stores localname as
			// prefix:localname.
			_cache.elements
			...
			var inlineHighlightTypes = [ 'nonFraction',
				'nonNumeric' ];
			_cache.hidden
				.each(function(index, element) {
		...
		_cache.elements.each(function(index, element) {
		    ...
		    if (element.attr('name') == name
			    && element.attr('contextRef') == context) {
		...
		App.showMesssage("getElementsByNegative");
		alert("getElementsByNegative");
		var results = $();
		...
		_cache.elements
			.each(function(index, element) {
				...
				var prefix = self.nsLookupByValue(
					self.namespaces.xbrli, true);
				for (var z = 0; z < result.MetaLinks_list.MetaLinks.elements.tag.length; z++) {
				    if ((result.MetaLinks_list.MetaLinks.elements.tag[z]['-id'] == xbrId)
					    && (result.MetaLinks_list.MetaLinks.elements.tag[z]['-xbrltype'] == prefix
						    + ':monetaryItemType')) {
					count++;
					results.push(element);
				    }
		...
		var ns = cbe.Utils.nsLookupByValue(_inlineDoc[0],
			self.namespaces.xbrli, true);
		if (ns) {
		    results = _cache.header
			    .find('*')
			    .filter(
				    function() {
					for (var i = 0; i < ns.length; i++) {
					    if (this.nodeName.toLowerCase() == ns[i]
						    + ":context") {
		...
		if (ns) {
		    for (var i = 0; i < ns.length; i++) {
			result = _cache.header.find(
				ns[i] + '\\:context, context').filter(
				function() {
	    ...
	    this.updateContextAxisForSearch = function(contextid,
		    dimensionNameForSearch) {
		...
		$.each(_cache.contextIDsForAxis, function(pos, val) {
		    if (val === contextid) {
			getContextFromCacheArray
				.push(_cache.dimensionNameforAxis[pos])
		    }
		...
		if (currentContext) {
		    segments = currentContext.find(self.instancePrefix
			    + "\\:segment, segment");
		}
		return segments;
	    };
		...
		items = _cache.header.find('*').filter(
			function() {

			    var xbrlchek = (this.nodeName.toLowerCase())
		...
		items = _cache.header
			...
			.filter(
				function() {
					...
					if (this.nodeName.toLowerCase() == self.instancePrefix[i]
						+ ":unit") {
					    return this.nodeName.toLowerCase() == self.instancePrefix[i]
						    + ":unit";
					}
				    }
				});
		...
		return cbe.Utils
			.nsLookupByValue(_inlineDoc[0], key, prefixOnly);
	    };
		...
		var schemaRef = null;
		var linkPrefix = self.nsLookupByValue(self.namespaces.link,
			true);
		...
		if (linkPrefix) {
		    ...
		    _cache.header
			    ...
			    .each(
					...
					var e = $(element);
					if (element.nodeName.toLowerCase() == linkPrefix
						+ ':schemaref') {
		...
		var note = '';
		var relationship = _cache.header.find("relationship[fromRefs='"
			+ id + "']");
	    ...
	    this.getSelectedCustomData = function(idgiven, parent,
		    cnamegetting, callback) {
		...
		if (App.InlineDoc.getMetaMoreData() != undefined) {
		    ...
		    if (label)
			callback.apply(parent, [ label ]);
		} else
		    return;

	    };

	    this.getSelectedLabel = function(idgiven, parent, cnamegetting,
		    callback) {
		...
		this.getReportFromURI(role, parent, function(report) {
		    if (report.shortName && callback)
			callback.apply(parent, [ report.shortName ]);
		});
	    };

	    this.getRoleLongName = function(role, parent, callback) {
		this.getReportFromURI(role, parent, function(report) {
		    if (report.longName && callback)
			callback.apply(parent, [ report.longName ]);
		});
	    };

	    this.getReferencesfromJson = function(cnamegetting, parent,
		    callback) {
		if (!this.getMetaData())
		    return;
		var authrefval = this.getMetaRefs();
		if (authrefval) {
		    callback.apply(parent, [ authrefval ]);
		}
		;
	    };
		...
		for ( var index in calculationItems) {
			...
			this.getSelectedLabel(calculation.parentTag, this,
				null, function(label) {
...
//		    throw new CustomNotFoundError(null, fallback);
	...
	cbe.XBRLDoc = function() {
		...
		if (_filingDocs.schema) {
		    ...
		    var nsTarget = cbe.Utils.nsLookupByKey(doc,
			    "targetNamespace");
		    if (nsTarget) {
		...
		if ((idPrefix == self.standardTaxonomy || idPrefix == _customPrefix)
			&& _filingDocs.label) {

		    var locEleName = 'loc', labelArcEleName = 'labelArc', labelEleName = 'label';
		    var prefix = cbe.Utils.nsLookupByValue(_filingDocs.label
			    .find(':root')[0], _namespaces.link, true);

		    // find loc
		    loc = _filingDocs.label
			    .find('*')
			    .filter(
				    function() {
					...
					var href = $(this).attr(
						prefix + ':href');
					return href
						&& (this.nodeName == locEleName || this.nodeName == prefix
							+ ':' + locEleName)
						&& href.indexOf(idStr,
							href.length
								- idStr.length) !== -1;
		    ...
		    // find arc(s)
		    arcs = _filingDocs.label
			    .find('*')
			    .filter(
				    function() {

					return (this.nodeName == labelArcEleName || this.nodeName == prefix
						+ ':' + labelArcEleName)
						&& $(this).attr(
							prefix + ':from') == labelId;
				    });
		    arcs
				...
				// get the labels
				labels = _filingDocs.label
					.find('*')
					.filter(
						function() {

						    return (this.nodeName == labelEleName || this.nodeName == prefix
							    + ':'
							    + labelEleName)
							    && $(this)
								    .attr(
		...
		if ((idPrefix == self.standardTaxonomy || idPrefix == _customPrefix)
		    ...
		    // find loc
		    loc = _filingDocs.label.find('loc[' + prefix
			    + '\\:href$="#' + id + '"]');
		    labelId = loc.attr(prefix + ':label');

		    // find arc(s)
		    arcs = _filingDocs.label.find('labelArc[' + prefix
			    + '\\:from="' + labelId + '"]');
			...
			// get the labels
			label = _filingDocs.label.find(
				'label[' + prefix + '\\:label= "' + arcId
					+ '"]').filter(function() {
		...
		alert('getRoleDefForSection');
		var item, roleTypeEleName = 'roleType', defEleName = 'definition';

		if (_filingDocs.schema) {

		    var prefix = cbe.Utils.nsLookupByValue(_filingDocs.schema
			    .find(':root')[0], _namespaces.link, true);

		    // find loc
		    var link = _filingDocs.schema
			    .find('*')
			    .filter(
				    function() {
					...
					return (this.nodeName == roleTypeEleName || this.nodeName == prefix
						+ ':' + roleTypeEleName)
						&& roleURI
							.substring(roleURI.length
								- sectionStr.length) == sectionStr;
				    });
		    if (link.length == 1) {

			var def = link.find('*').filter(
				function() {

				    return this.nodeName == defEleName
		...
		alert('getFilingDocLoadStatuses');
		var statuses = {};
...
$(document)
	.ready(
		function() {
			    ...
			    .bind(
					...
					if ((code == 13) || (code == 32)) { // Enter
						...
						if (panel.hasClass("visible")) {
							...
							App_Find.Results._cachescrollDestination
								.scrollIntoView();
						    }
		    ...
		    $(
			    '#information,#instanceFileLi,#instanceFileIELi,#instanceAndCustomFile,#btn-reports,#btn-settings,#btn-help')
		    ...
		    $('#taggedDataResetColor')
			    .bind(
					...
					if ((code == 13) || (code == 32)) {
					    var icons = document
						    .getElementsByClassName("simpleColorDisplay");
					    for (var i = 0; i < icons.length; i++) {
						$(icons[0]).attr("tabindex", 0);
						$(icons[0])
							.attr("aria-label",
								"Tagged Data Color Selector");
						icons[0].click();
					    }
					    $('.simpleColorChooser').attr(
						    "tabindex", 0);
					    $('.simpleColorCell').attr(
						    "tabindex", 0);
					    // var simpleClorCellZeroId=
					    // $('.simpleColorCell')[0].id;
					    // var
					    // hexcode="#"+simpleClorCellZeroId;
					    // $('.simpleColorCell').attr("aria-label",hexcode);
					    $('.simpleColorCell')
						    .attr("aria-label",
							    "Inside Tagged Data Color Selector");
					    var count = 0;
					    $('.simpleColorCell')
						    .bind(
							    'keydown',
								...
								if (code == 9) {
									...
									if (j == (count)) {
									    // var
									    // simpleClorCellIds=
									    // $('.simpleColorCell')[j].id;
									    // var
									    // hexcode="#"+simpleClorCellIds;
									    // $('.simpleColorCell').attr("aria-label",hexcode);
									    $(
										    '.simpleColorCell')
										    .attr(
		    ...
		    $('#setting-border-highlight-color')
			    .bind(
				    'keydown',
				    function(e) {
					var code = e.keyCode || e.which;
		    ...
		    $('#searchResultsResetColor')
			    .bind(
					...
					if ((code == 13) || (code == 32)) {
					    ...
					    $('.simpleColorCell')
						    .attr("aria-label",
							    "Inside Search Results Color Selector");
					    var count = 0;
					    $('.simpleColorCell')
						    .bind(
							    'keydown',
		    ...
		    $('#selectedFactResetColor')
			    .bind(
					...
					if ((code == 13) || (code == 32)) {
					    ...
					    $('.simpleColorCell')
						    .attr("aria-label",
							    "Inside Selected Fact Color Selector");
					    var count = 0;
					    $('.simpleColorCell')
						    .bind(
							    'keydown',
		    ...
		    $('#btn-reports')
			    .on(
					...
					$('#app-panel-reports-container').show(
						'slide');
			    ...
			    .on(
					...
					if (panel.hasClass("visible")) {
					    $('#menuBtn-reports')
						    ...
						    .addClass(
							    'icon-expand-more');
					    $('#menuBtn-reports').attr("title",
						    "Expand Tagged Sections");
					    $('#menuBtn-reports').attr(
						    "aria-label",
						    "Expand Tagged Sections");
						...
						App_Find.TaggedSection._cachescrollDestination
							.scrollIntoView();
...
jQuery(function($) {
    ...
    $(document).on(
	    ...
	    '.close-icon',
	    function() {
		App.showSpinner1($('#mainDiv'),
			function() {
			    App_Find.Results.load();
			    if ($('#app-panel-reports-container')
				    .css('display') == 'block') {
				App_Find.TaggedSection.refreshTaggedSection($(
					'#search-input').val());
			    }
